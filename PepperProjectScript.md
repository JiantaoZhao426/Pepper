...
- [Scripts of the Pepper Resequencing Project](#scripts-of-the-pepper-resequencing-project)
...
- [Table of Contents](#table-of-contents)
  ...
  - [SNP calling and evaluation](#snp-calling-and-evaluation)
    ...
    - [Pi diversity comparison between 1314 and 500 core collection](#pi-diversity-comparison-between-1314-and-500-core-collection)
  ...
  - [Nucleotide diversity](#nucleotide-diversity)
    ...
    - [Nucleotide diversity without step](#nucleotide-diversity-without-step)
  ...
  - [Perl scripts](#perl-scripts)
    ...
    - [merge.pl](#mergepl)
  - [R scripts](#r-scripts)
    - [World map of samples](#world-map-of-samples)
    - [PCA](#pca)
    - [IQ-tree plot using ggtree](#iq-tree-plot-using-ggtree)
    - [GWAS](#gwas-1)
    - [XP-CLR](#xp-clr)
    - [Genome-wide distribution of fd](#genome-wide-distribution-of-fd)
      - [Chr06 candidate region pi plot](#chr06-candidate-region-pi-plot)
...
### Pi diversity comparison between 1314 and 500 core collection

```bash
# Working directory
cd /data/zhaojiantao/pepper/pepper_survey

# Split the chromosomes and filter the data, aiming to greatly reduce the number of SNPs
bcftools view 1314.vcf.gz --regions Superscaffold1_54_332676983 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr01.vcf
bcftools view 1314.vcf.gz --regions Superscaffold2_27_177319215 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr02.vcf
bcftools view 1314.vcf.gz --regions Superscaffold3_40_289790774 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr03.vcf
bcftools view 1314.vcf.gz --regions Superscaffold4_26_248932513 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr04.vcf
bcftools view 1314.vcf.gz --regions Superscaffold5_64_254874144 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr05.vcf
bcftools view 1314.vcf.gz --regions Superscaffold6_82_253233553 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr06.vcf
bcftools view 1314.vcf.gz --regions Superscaffold7_54_266382521 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr07.vcf
bcftools view 1314.vcf.gz --regions Superscaffold8_30_174326481 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr08.vcf
bcftools view 1314.vcf.gz --regions Superscaffold9_43_278428952 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr09.vcf
bcftools view 1314.vcf.gz --regions Superscaffold10_31_210332287 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr10.vcf
bcftools view 1314.vcf.gz --regions Superscaffold11_37_275185330 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr11.vcf
bcftools view 1314.vcf.gz --regions Superscaffold12_60_260676116 -i 'F_MISSING<0.3 ' --types snps -m 2 -M 2 --threads 10 > Chr12.vcf

# Index the file
for i in $(seq -w 1 12); do tabix Chr$i.vcf ; done
...
for i in $(seq -w 1 12); do
    ...
    gatk VariantFiltration \
        ...
        -O Chr$i.filt1.vcf
    # Extract the hard filtered
    bcftools view Chr$i.filt1.vcf --threads 10 \
        --apply-filters PASS \
        > Chr$i.filt2.vcf
done
...
for i in $(seq -w 1 12); do
bcftools reheader --samples sample.id.correction.txt Chr$i.filt2.vcf -o Chr$i.filt3.vcf
done

# Calculate windowed pi for the groups from the 1314 accessions with 100 time permutations
for i in $(seq -w 1 100); do
...
for i in $(seq -w 1 12); do
    ...
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/PUB.list | head -38) \
        --window-pi 100000 --out PUB.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/CHA.list | head -17) \
        --window-pi 100000 --out CHA.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/BAB.list | head -9) \
        --window-pi 100000 --out BAB.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/BAP.list | head -109) \
        --window-pi 100000 --out BAP.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/GLA.list | head -19) \
        --window-pi 100000 --out GLA.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/ANN.list | head -75) \
        --window-pi 100000 --out ANN.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/FRU.list | head -79) \
        --window-pi 100000 --out FRU.Random$j.Chr$i &
    done
done
...
for i in $(seq -w 1 12); do
    for j in $(seq -w 1 100); do
    vcftools --vcf Chr$i.filt3.vcf \
        --keep <(sort --random-sort Pop.list.all/CHN.list | head -105) \
        --window-pi 100000 --out CHN.Random$j.Chr$i &
    done
done

# Calculate the average value of permuatations
for i in PUB CHA BAB BAP FRU CHN GLA ANN; do
    for j in $(seq -w 1 100); do
    cat $i.Random$j.Chr* | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > $i.Random$j.mean.pi
    done
done

# Combine the mean values
for i in PUB CHA BAB BAP FRU CHN GLA ANN; do
cat $i.Random*.mean.pi > $i.mean.pi
done

# Calculate windowed pi for the groups from the 1314 accessions
for group in PUB CHA BAB BAP FRU CHN GLA ANN; do
    # Calculate per-site nucleotide diversity
    for i in $(seq -w 1 12); do
        vcftools --vcf Chr$i.filt3.vcf \
            --keep Pop.list.all/$group.list --window-pi 100000 --out All.$group.Chr$i &
    done
done

# Calculate windowed pi for the groups from the core accessions
for group in PUB CHA BAB BAP FRU CHN GLA ANN; do
    # Calculate per-site nucleotide diversity
    for i in $(seq -w 1 12); do
        vcftools --vcf Chr$i.filt3.vcf \
            --keep ../vg/Pop.list/$group.list --window-pi 100000 --out Core.$group.Chr$i &
    done
done

# Combine the output for the All
cat All.PUB.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.PUB.mean.pi
cat All.CHA.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.CHA.mean.pi
cat All.BAB.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.BAB.mean.pi
cat All.BAP.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.BAP.mean.pi
cat All.GLA.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.GLA.mean.pi
cat All.ANN.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.ANN.mean.pi
cat All.FRU.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.FRU.mean.pi
cat All.CHN.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > All.CHN.mean.pi

# Combine the output for the Core
cat Core.PUB.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.PUB.mean.pi
cat Core.CHA.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.CHA.mean.pi
cat Core.BAB.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.BAB.mean.pi
cat Core.BAP.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.BAP.mean.pi
cat Core.GLA.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.GLA.mean.pi
cat Core.ANN.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.ANN.mean.pi
cat Core.FRU.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.FRU.mean.pi
cat Core.CHN.Chr*.windowed.pi | grep -v PI | grep -v nan | awk '{ sum += $5 } END { print sum / NR}' > Core.CHN.mean.pi

# T-test for PUB
R
data <- read.table("PUB.mean.pi", header=FALSE)
summary(data$V1)
t.test(x=data, mu=1.70301e-05, conf.level=0.95)
...
### Nucleotide diversity without step
...
# Calculate per-site nucleotide diversity
for i in PUB CHA BAB BAP GLA ANN FRU CHN ; do
    for j in $(seq -w 1 12); do
        vcftools --gzvcf ../../04SNP/Miss_0.3/Chr$j.SNP.vcf.gz --keep ../../Pop.list/$i.list --window-pi 100000 --out $i.Chr$j &
    done
done
...
cat FRU.Chr*.windowed.pi | grep -v CHROM | awk '{print $5,"G"}' > FRU.windowed.pi-1
cat CHN.Chr*.windowed.pi | grep -v CHROM | awk '{print $5,"H"}' > CHN.windowed.pi-1
cat *windowed.pi-1 | sed '1i\PI Group' | sed 's/ /\t/g' > Pi.windowed.txt
...
library(raster)
library(maptools)
Map <- read.table("Data/Map.txt", head=T,sep="\t") 
Map_reshape <- cast(Map,Latitude+Longitude~Sub.population) 
Map_reshape <- as.data.frame(Map_reshape)

pdf("Map.pdf",w=15,h=5)
Map_reshape.new <- cast(Map,Latitude+Longitude~Sub.population.new) 
Map_reshape.new <- as.data.frame(Map_reshape.new)
pdf("Map.new.pdf",w=15,h=5)
mapPies(Map_reshape.new, 
        nameX="Longitude", nameY="Latitude",
        zColours=c("#00B0F0", "#C65911", "#7030A0", 
                   "#548235","#FFC000", "#D400E3", "#DFE349", 
                   "#FF0000"), ratio =1,
        nameZs=c("C.annuum","C.bac.baccatum","C.bac.pendulum",
                 "C.chacoense","C.chinense","C.frutescens", "C.ann.glabriusculum",
                 "C.pubescens"),
        mapRegion='world', symbolSize=1.1, 
        borderCol = "black",oceanCol="white",
        landCol="grey95",lwdSymbols="NA")
dev.off()
...
# Eigen value
Eigen <- fread("Data/Eigenvalue.txt", sep="\t", header = T)
jpeg("Eigen.jpeg", height = 6, width = 6, units="cm", res = 300)
ggplot(Eigen, aes(x=PCs, y=Eigenvalue)) +
...
# PCA
PCA <- fread("Data/PCA.txt", sep="\t", header = T)
pdf("PC1_PC2.pdf", h =3, w=5)
ggplot(PCA, aes(x=PC1, y=PC2,shape = New, color = New, group = New)) +
  scale_shape_manual(values=seq(0,15)) + geom_point() +
  xlab("PC1 (26.84%)") +ylab("PC2 (16.11%)") +
  scale_color_manual(values=c("#DFE349", "#00B0F0","#C65911", "#7030A0",
                              "#548235","#FFC000",
                              "#D400E3","black", "#FF0000")) 
dev.off()
...
# Prune
tree <- read.tree("Data/504.4DTV.LD1.min4.phy.txt")
write.table(tree$tip.label, "tip.label504.LD1.txt")
# Read the group information
group <- read.table("Data/504.4DTV.LD1.min4.pop.txt", header = TRUE)
my_info <- data.table(tip_lbs = tree$tip.label,
                      groupC = group$New)
my_info2 <- data.table(tip_lbs = tree$tip.label,
                      groupC = group$New2)
...
levels(attributes(tree_grC)$group)
attributes(tree_grC)$group <- factor(x = attributes(tree_grC)$group, 
                                     levels = c("Tomato","C.pubescens","C.chacoense",
                                                "C.bac.baccatum","C.bac.pendulum",
                                                "C.ann.glabriusculum","C.annuum",
                                                "C.galapagoense","C.frutescens","C.chinense","0"))
# check all group labels
levels(attributes(tree_grC)$group)

# Define the group color
my_cols <- c("Black","#FF0000","#548235",
             "#C65911","#7030A0",
             "#DFE349","#00B0F0",
             "Black","#D400E3", 
             "#FFC000", "black")
...
pdf("Phylogeny-vg_raw.pdf", height = 50, width = 20)
ggtree(tree_grC, branch.length="none",mapping = aes(color = group))+ 
  geom_tiplab(size = 0.7) + scale_y_reverse() + 
  scale_color_manual(name = 'Taxonomy', values = my_cols)
dev.off()

grC <- split(my_info2$tip_lbs, my_info2$groupC)
# Use groupOTU for taxa classification
tree_grC <- ggtree::groupOTU(tree, grC)
# check all group labels
levels(attributes(tree_grC)$group)
...
pdf("Phylogeny-vg_updated.pdf", height = 50, width = 20)
ggtree(tree_grC, branch.length="none",mapping = aes(color = group))+ 
  geom_tiplab(size = 0.7) + scale_y_reverse() + 
  scale_color_manual(name = 'Taxonomy', values = my_cols)
dev.off()
...
# GLA.ANN.GWAS.FruitLength
GLA.ANN.GWAS.FruitLength <- fread("Data/GLA.ANN.FruitLength.GWAS.txt", header = TRUE)
jpeg("GLA.ANN.FruitLength.jpeg", height = 8, width = 20, units="cm", res = 300)
manhattan(GLA.ANN.GWAS.FruitLength, chr="CHR", bp="BP", snp="SNP", p="P", 
          col = c("orange", "darkgreen"), cex=0.1, ylim=c(0,8),
          genomewideline = 6.29, suggestiveline = FALSE)
...
# GLA.ANN.GWAS.FruitWidth
GLA.ANN.GWAS.FruitWidth <- fread("Data/GLA.ANN.FruitWidth.GWAS.txt", header = TRUE)
jpeg("GLA.ANN.FruitWidth.jpeg", height = 8, width = 20, units="cm", res = 300)
manhattan(GLA.ANN.GWAS.FruitWidth, chr="CHR", bp="BP", snp="SNP", p="P", 
          col = c("orange", "darkgreen"), cex=0.1, ylim=c(0,8),
          genomewideline = 6.29, suggestiveline = FALSE)
...
```r
# GLA.ANN
GLA.ANN.xpclr <- fread("Data/GLA.ANN.out.xpclr.txt", header = TRUE)
snps <- read.table("Data/snpsofInterest.txt", header = FALSE)
as.character(snps)
jpeg("XPCLR.GLA.ANN.jpeg", height = 8, width = 20, units="cm", res = 300)
manhattan(GLA.ANN.xpclr, chr="Chr", bp="Pos", snp="SNP", p="XPCLR", 
          col = c("orange", "darkgreen"), logp = FALSE, cex=0.1, 
          genomewideline = 44.591, suggestiveline = FALSE,
          ylab="XPCLR",highlight = snps)
dev.off()

# BAB.BAP
BAB.BAP.xpclr <- fread("Data/BAB.BAP.out.xpclr.txt", header = TRUE)
snps <- read.table("Data/BAB.BAP.overlapped.snps.txt", header = FALSE)
snps.trans <- t(snps)
as.character(snps.trans)
jpeg("XPCLR.BAB.BAP.jpeg", height = 8, width = 20, units="cm", res = 300)
manhattan(BAB.BAP.xpclr, chr="Chr", bp="Pos", snp="SNP", p="XPCLR", 
          col = c("purple", "darkorange"), logp = FALSE, cex=0.1, 
          genomewideline = 3.548, suggestiveline = FALSE,
          ylab="XPCLR", ylim=c(25,0),highlight = snps.trans)
dev.off()

```

### Genome-wide distribution of fd

```r
library(qqman)
library(data.table)
fd <- fread("Data/PUB.BAC.FRUCHN.ANN.boxplot.vg.out.txt", header = TRUE)
# Manhattan plot
jpeg("ABBABABA.BAC.FRUCHN.vg.jpeg", height = 8, width = 9.3, units="cm", res = 300)
manhattan(fd, chr="Chr", bp="Pos", snp="SNP", p="fd", 
          col = c("orange", "darkgreen"), logp = FALSE, cex=0.2, 
          genomewideline = 0.1138, suggestiveline = FALSE,
          ylab="fd")
dev.off()

```

#### Chr06 candidate region pi plot

```r
library(ggplot2)
pdf("BAC2FRUCHN.Chr06.region.pi.vg.pdf", height = 2.5, width = 4.5)
BAC2FRUCHN.pi <- fread("Data/BAC2FRUCHN.Chr06.region.pi.vg.txt", header = TRUE)
ggplot(BAC2FRUCHN.pi, aes(x=Pos/1e6, y=Pi, color=Group)) +
  geom_line() + 
  ylim(0,0.016) +xlim(243.2,246.15) +
  scale_color_manual(values=c("#00B0F0","#C65911","#7030A0", 
              "#FFC000","#D400E3","#FF0000")) +
  geom_segment(aes(x = 244.011, y = 0.010, xend = 244.353, yend = 0.010), col = "blue", size = 2) +
  geom_segment(aes(x = 244.463, y = 0.011, xend = 244.817, yend = 0.011), col = "blue", size = 2) +
  geom_segment(aes(x = 245.352, y = 0.011, xend = 245.356, yend = 0.011), col = "blue", size = 2) +
  geom_segment(aes(x = 245.404, y = 0.011, xend = 245.408, yend = 0.011), col = "blue", size = 2)
dev.off()
